...
KERNEL_REPO_URL="${KERNEL_REPO_URL:-$KIT_KERNEL_REPO_URL}"
. ../DISTRO_SPECS
...
if [ "$USR_SYMLINKS" = "yes" ] ; then
    ...
    rm -f $ZDRVSFS
    usrmerge zdrv_merge_tmp/extracted 0
    mksquashfs zdrv_merge_tmp/extracted $ZDRVSFS $SFSCOMP
    rm -rf zdrv_merge_tmp
fi
cd ..
